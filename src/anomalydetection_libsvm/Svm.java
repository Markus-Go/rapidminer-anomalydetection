...
class ETA_ONE_CLASS_Q extends Kernel
{

	private final Cache cache;
	private final float[] QD;
	private final byte[] eta;

	ETA_ONE_CLASS_Q(svm_problem prob, svm_parameter param, byte[] eta_)
	{
		super(prob.l, prob.x, param);
		cache = new Cache(prob.l, (long) (param.cache_size * (1 << 20)));
		QD = new float[prob.l];
		eta = eta_.clone();
		for(int i=0;i<prob.l;i++)
			QD[i] = (float) (eta[i] * kernel_function(i, i));
	}
	...
	float[] get_Q(int i, int len)
	{
		...
		{
			for(int j=start;j<len;j++)
				data[0][j] = (float) (eta[i] * eta[j] * kernel_function(i, j));
		}
		return data[0];
	}
...
public class Svm {
	...
	private static void solve_robust_one_class(svm_problem prob,
		...
		for(i=0;i<l;i++) {
			...
			double kavg = 0;
			for(int j=0;j<l;j++) {
				kavg += Kernel.k_function(prob.x[i], prob.x[j], param);
			}
			D[i] = Math.abs(Kernel.k_function(prob.x[i], prob.x[i], param) - 2
					* kavg / l);
			Dmax = D[i] > Dmax ? D[i] : Dmax;
		}

		for(i=0;i<l;i++) {
			p[i] = param.lambda * D[i] / Dmax;
			ones[i] = 1;
		}

		Solver s = new Solver();
		s.Solve(l, new ONE_CLASS_Q(prob, param), p, ones, alpha, 1.0, 1.0,
				param.eps, si, param.shrinking);
	}

	private static void solve_eta_one_class(svm_problem prob,
											svm_parameter param, double[] alpha, Solver.SolutionInfo si) {
		// output SVs
		byte[] eta = new byte[prob.l];
		Arrays.fill(eta, (byte)1);
		int numberOfIterations = 0;
		int maxIteratoions=100;
		boolean change = true;
		int changed=2;
		while(true) {
			if(numberOfIterations == maxIteratoions || !change){
				if(numberOfIterations==maxIteratoions)System.out.println("Didnt converge! "+changed);
				break;
				
			}
			solve_eta_one_class_subproblem(prob, param, alpha, eta, si);
			int nSV = 0;
			...
			int i;
			svm_node[][] SV = new svm_node[nSV][]; // support vectors
			double[] sv_coef = new double[nSV];
			byte[] sv_eta = new byte[nSV];
			...
			double sum = 0;
			Pair[] order = new Pair[prob.l];
			for(i=0;i<prob.l;i++) {
				sum = 0;
				for(j=0;j<nSV;j++)
					sum += sv_eta[j] * sv_coef[j]
							* Kernel.k_function(prob.x[i], SV[j], param);
				order[i] = new Pair();
				order[i].index = i;
				order[i].value = f.rho - sum;
			}
			...
			changed =0;
			int index;
			int m =(int) ((1.0 - param.nu) * prob.l);
			for(i=0;i<m;i++) {
				index = order[i].index;
				change = change || eta[index] == 0;
				if(eta[index]==0)
					changed++;
				eta[index] =(byte) 1;
			}
	...
	private static void solve_eta_one_class_subproblem(svm_problem prob,
														svm_parameter param, double[] alpha, byte [] eta , Solver.SolutionInfo si) {
		int l = prob.l;
		double[] zeros = new double[l];
		byte[] ones = new byte[l];
		int i;
		...
		Solver s = new Solver();
		s.Solve(l, new ETA_ONE_CLASS_Q(prob, param, eta), zeros, ones, alpha, 1.0, 1.0,
				param.eps, si, param.shrinking);
	}
		...
		double Cp, double Cn)
	{
		...
		switch(param.svm_type)
		{
			...
			case svm_parameter.ONE_CLASS:
				...
				break;
			case svm_parameter.ROBUST_ONE_CLASS:
				solve_robust_one_class(prob, param, alpha, si);
				break;
			case svm_parameter.ETA_ONE_CLASS:
				solve_eta_one_class(prob, param, alpha, si);
				break;
	...
	public static svm_model svm_train(svm_problem prob, svm_parameter param)
	{
		...
		if(param.svm_type == svm_parameter.ONE_CLASS ||
		   ...
		   param.svm_type == svm_parameter.ROBUST_ONE_CLASS ||
		   param.svm_type == svm_parameter.ETA_ONE_CLASS)
		{
			...
			int nSV = 0;
			// counts the number of bound support vectors for one-class SVMs 
			int nBSV=0;
			int i;
			for(i=0;i<prob.l;i++)
			...
			model.l = nSV;
			model.nBSV = nBSV;
			model.nBSVIndicies = new int[nBSV];
	...
	public static void svm_predict_values(svm_model model, svm_node[] x, double[] dec_values)
	{
		if(model.param.svm_type == svm_parameter.ONE_CLASS ||
		   ...
		   model.param.svm_type == svm_parameter.ROBUST_ONE_CLASS ||
		   model.param.svm_type == svm_parameter.ETA_ONE_CLASS)
		{
	...
	public static String svm_check_parameter(svm_problem prob, svm_parameter param)
	{
		...
		if(svm_type != svm_parameter.C_SVC &&
		   ...
		   svm_type != svm_parameter.ROBUST_ONE_CLASS &&
		   svm_type != svm_parameter.ETA_ONE_CLASS)
		...
		if(svm_type == svm_parameter.ROBUST_ONE_CLASS)
			if(param.lambda < 0)
				return "lambda < 0";
